...
class ConversationalRAG:
    def __init__(self, session_id: str, retriever):
        try:
            ...
            raise DocumentPortalException("Failed to initialze ConversationalRAG", sys)
    ...
    def load_retriever_from_faiss(self, index_path: str):
        try:
            ...
            self.log.error("faield to load retreiver from FAISS", error=str(e))
            raise DocumentPortalException("Error loading retriever from FAIS", sys)
        
    def invoke(self, user_input:str)->str:
        try:
            response = self.chain.invoke(
                { "input":user_input}, config={"configurable": {"session_id": self.session_id}}
                )
