...
class FaissManager:
    ...
    def add_documents(self,docs: List[Document]):
        if self.vs is None:
            raise RuntimeError("Call load_or_create() before add_documents_id")
...
class DocumentComparator:
    ...
    def save_uploaded_files(self, reference_file, actual_file):
        try:
            ...
            self.log.info("Files saved", reference=str(ref_path), actual=str(act_path), session_id=self.session_id)
    ...
    def read_pdf(self, pdf_path: Path)-> str:
        try:
            ...
            raise DocumentPortalException("Errror readind PDF", e) from e       
    ...
    def combine_documents(self)-> str:
        try:
            ...
            self.log.error("Error combining documents", error = str(e), session_id=self.session_id)
